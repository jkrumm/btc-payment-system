...
const subscribe = () => {
  connection.then(() => {
    subscriber = stompClient.subscribe('/topic/tx', data => {
...
const connect = () => {
  ...
  const headers = {};
  let url = '//' + loc.host + baseHref + '/websocket/tx';
...
export default store => next => action => {
  if (action.type === SUCCESS(AUTH_ACTIONS.GET_SESSION)) {
    ...
    if (!alreadyConnectedOnce && isUser) {
      subscribe();
      receive().subscribe(tx => {
        return store.dispatch({
          type: WALLET_ACTIONS.TX_WEBSOCKET_ACTIVITY_MESSAGE,
          payload: tx,
        });
      });
    }
